...
flip_var :: forall a. Char -> Reader BoolMap a -> Reader BoolMap a
flip_var c xM = do
   env <- ask
   let b = env Map.! c
   local (Map.insert c (not b) env) xM

insert_var :: forall a. Char -> Reader BoolMap a -> Reader BoolMap a
insert_var c xM = do
   env <- ask
   local (Map.insert c (True) env) xM
...
evalDumb :: Expr -> Reader BoolMap Bool
evalDumb e = do
  env <- ask
...
satisfiableVarsM xs e = case xs of 
  ...
  x:xs' -> do
    ...
    --(insert_var x) $ do 
      --(flip_var x) $ do
    env2 <- ask
    return (unReader (satisfiableVarsM xs' e) (Map.insert x True env1) ++ unReader (satisfiableVarsM xs' e) (Map.insert x False env1)) 
      --let b2 = env Map.! x
      --unReader (evalM b) 
      --unReader satisfiableVarsM (xs' (Map.insert x (b1) env ++ 
      --let x2 = env Map.! x
      
      --return [Map.fromList([('a', True)])]
    --env <- ask
    --let b = env Map.! x
    --local (Map.insert x (not b) env)
